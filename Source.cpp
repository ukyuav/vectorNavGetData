...
int main(int argc, char *argv[])
{
	...
	int fileDesc = _open(argv[1], _O_BINARY, _O_RDONLY);
	if (fileDesc == -1) {
		perror("Can't open file: ");
	}
	int fileSize = _lseek(fileDesc, 0, SEEK_END); // Find filesize
	_lseek(fileDesc, 0, SEEK_SET); //Reset to the beginning
	char buffer[PACKETSIZE];
	int numRead = _read(fileDesc, buffer, PACKETSIZE);
	if (numRead == -1) {
	...
	while (numRead == PACKETSIZE) {
		...
		//if((byte)buffer[0] == 0xFA){
			...
			byte syncByte;
			long offset = 0;
			_read(fileDesc, &syncByte, 1);
			while ((syncByte != 0xFA) && (offset < fileSize)) {
				offset =_lseek(fileDesc, 0, SEEK_CUR);
			...
			_lseek(fileDesc, -1, SEEK_CUR);
			countCorrupt++;
		}

		numRead = _read(fileDesc, buffer, PACKETSIZE);
		if (numRead == -1) {
